...
/**
 * This data format provides continuous data transmission of a 5 byte data packet sent 75 times per second. The data
 * packet includes real-time data of: 8-bit waveform value, beat-to-beat SpO2 value, SpO2 and Pulse Rates values
 * formatted for both recording and display purposes, status of the measurement and battery.
 * 
...
public class DataFormat2FrameSerialiser {
	...
	public static DataFormat2Frame read(InputStream inputStream, int frameCount) throws IOException {
		...
		short plethmograph = IOUtil.readUByte(inputStream);
		calculated_check_sum = (calculated_check_sum + plethmograph) & 0xff;
		...
		return new DataFormat2Frame(artifact, out_of_track, sensor_alarm,
				perfusion, sync, plethmograph, val_byte);
	}
}
